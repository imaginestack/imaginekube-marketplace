...
subdomain="$CLUSTER_ID.k8s.imaginekube.com"
email="$EMAIL"
...
helm upgrade --install imaginekube okteto/okteto-enterprise --namespace okteto -f https://raw.githubusercontent.com/imaginekube/kubernetes-marketplace/master/okteto/config.yaml --set email="$email" --set adminToken="$adminToken" --set subdomain="$subdomain" --set ingress.ip=$ingress --version 0.9.6
if [ ! $? -eq 0 ]; then
...
kubectl rollout status deployment/imaginekube-okteto-enterprise-api -n=okteto
kubectl rollout status statefulset/imaginekube-okteto-enterprise-buildkit -n=okteto
