...
3. With the service name, create an ingress as below:
   ```yaml
    ...
    spec:
      rules:
        - http:
          host: eventlistenername.your-cluster-id.k8s.imaginekube.com
            paths:
              - path: /
                backend:
...
This will open up http://eventlistenername.YOUR_CLUSTER_ID.k8s.imaginekube.com to trigger the service.  Ensure your event-listener has the necessary interceptor configuration in place to prevent unauthorized triggering of the pipeline.